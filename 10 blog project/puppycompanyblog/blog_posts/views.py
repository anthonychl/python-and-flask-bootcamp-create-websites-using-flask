...
# view
@blog_posts.route('/<int:blog_post_id>') # int:  makes sure that blog_post_id is treated as an integer not a string
...
def update(blog_post_id):
    ...
    if blog_post.author != current_user:
        abort(403) # 403 means Forbidden, aborting the update if the current user isnt the author of the post
    form = BlogPostForm()
    if form.validate_on_submit():
